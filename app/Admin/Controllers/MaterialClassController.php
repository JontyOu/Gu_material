<?php

namespace App\Admin\Controllers;

use App\Models\MaterialClasses;
use Encore\Admin\Controllers\AdminController;
use Encore\Admin\Form;
use Encore\Admin\Grid;
use Encore\Admin\Show;

class MaterialClassController extends AdminController
{
    /**
     * Title for current resource.
     *
     * @var string
     */
    protected $title = '分类管理';

    /**
     * Make a grid builder.
     *
     * @return Grid
     */
    protected function grid()
    {
        $grid = new Grid(new MaterialClasses());

        $grid->column('id', __('Id'));
        $grid->column('name', __('分类名称'));
        $grid->column('time', __('时间'));
        $grid->column('status', __('状态'));
//        $grid->column('is_del', __('Is del'));
//        $grid->column('created_at', __('Created at'));
//        $grid->column('updated_at', __('Updated at'));

        return $grid;
    }

    /**
     * Make a show builder.
     *
     * @param mixed $id
     * @return Show
     */
    protected function detail($id)
    {
        $show = new Show(MaterialClasses::findOrFail($id));

        $show->field('id', __('Id'));
        $show->field('name', __('Name'));
        $show->field('time', __('Time'));
        $show->field('status', __('Status'));
        $show->field('is_del', __('Is del'));
        $show->field('created_at', __('Created at'));
        $show->field('updated_at', __('Updated at'));

        return $show;
    }

    /**
     * Make a form builder.
     *
     * @return Form
     */
    protected function form()
    {
        $form = new Form(new MaterialClasses());

        $form->text('name', __('Name'));
        $form->number('time', __('Time'));
        $form->switch('status', __('Status'))->default(1);
        $form->switch('is_del', __('Is del'));

        return $form;
    }
}
